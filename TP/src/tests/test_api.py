...
def test_valid_request_returns_200(client, mocker):
    ...
    # C'est a dire qu'on remplace ce que renvoi la méthode triangulate_from_id par les valeur (0, 1, 2)
    mocker.patch("src.triangulator.triangulator.Triangulator.triangulate_from_id",
                 return_value=[(0, 1, 2)])
    valid_uuid = "123e4567-e89b-12d3-a456-426614174000"
    response = client.get("/triangulate/{uuid}".format(uuid=valid_uuid))
...
def test_pointset_not_found_returns_404(client, mocker):
    ...
    response = client.get("/triangulate/{uuid}".format(uuid=uuid_not_found))
...
def test_internal_error_returns_500(client, mocker):
    ...
    response = client.get("/triangulate/{uuid}".format(uuid=uuid_error))
...
def test_service_unavailable_returns_503(client, mocker):
    ...
    response = client.get("/triangulate/{uuid}".format(uuid=uuid_fail))
